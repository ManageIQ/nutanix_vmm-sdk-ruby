...
module NutanixVmm
  ...
  class VmmV40AhvConfigEmulatedNic < ApiModelBase
    attr_accessor :model
    ...
    def self.openapi_types
      {
        :'model' => :'VmmV40AhvConfigEmulatedNicModel',
        :'mac_address' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NutanixVmm::VmmV40AhvConfigEmulatedNic` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixVmm::VmmV40AhvConfigEmulatedNic`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@num_queues.nil? && @num_queues < 1
        invalid_properties.push('invalid value for "num_queues", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@num_queues.nil? && @num_queues < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] num_queues Value to be assigned
    def num_queues=(num_queues)
      if num_queues.nil?
        fail ArgumentError, 'num_queues cannot be nil'
      end

      if num_queues < 1
        fail ArgumentError, 'invalid value for "num_queues", must be greater than or equal to 1.'
      end
    ...
    def hash
      [model, mac_address, is_connected, num_queues].hash
    end
