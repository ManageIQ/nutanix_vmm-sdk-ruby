...
module NutanixVmm
  ...
  class VmmV40ImagesConfigFilter
    attr_accessor :type
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixVmm::VmmV40ImagesConfigFilter`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def category_ext_ids=(category_ext_ids)
      ...
      if category_ext_ids.length > 100
        fail ArgumentError, 'invalid value for "category_ext_ids", number of items must be less than or equal to 100.'
      end

      if category_ext_ids.length < 1
        fail ArgumentError, 'invalid value for "category_ext_ids", number of items must be greater than or equal to 1.'
      end
