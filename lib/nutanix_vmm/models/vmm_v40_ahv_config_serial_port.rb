...
module NutanixVmm
  ...
  class VmmV40AhvConfigSerialPort < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NutanixVmm::VmmV40AhvConfigSerialPort` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixVmm::VmmV40AhvConfigSerialPort`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@index.nil? && @index > 3
        invalid_properties.push('invalid value for "index", must be smaller than or equal to 3.')
      end
    ...
    def valid?
      ...
      return false if !@index.nil? && @index > 3
    ...
    def index=(index)
      ...
      if index > 3
        fail ArgumentError, 'invalid value for "index", must be smaller than or equal to 3.'
      end
    ...
    def hash
      [tenant_id, ext_id, links, is_connected, index].hash
    end
