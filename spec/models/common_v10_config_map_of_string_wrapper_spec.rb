=begin
#Nutanix Virtual Machine Management APIs

#Manage the life-cycle of virtual machines hosted on Nutanix

The version of the OpenAPI document: 4.0

Generated by: https://openapi-generator.tech
Generator version: 7.16.0

=end

require 'spec_helper'
require 'json'
require 'date'

# Unit tests for NutanixVmm::CommonV10ConfigMapOfStringWrapper
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe NutanixVmm::CommonV10ConfigMapOfStringWrapper do
  #let(:instance) { NutanixVmm::CommonV10ConfigMapOfStringWrapper.new }

  describe 'test an instance of CommonV10ConfigMapOfStringWrapper' do
    it 'should create an instance of CommonV10ConfigMapOfStringWrapper' do
      # uncomment below to test the instance creation
      #expect(instance).to be_instance_of(NutanixVmm::CommonV10ConfigMapOfStringWrapper)
    end
  end

  describe 'test attribute "map"' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
